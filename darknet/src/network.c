...
void forward_network_stripped_mp(network net, network_state state, int nW, int nThreads)
{
    ...
    for(i = 0; i < net.n; ++i){
        ...
        if(l.type == CONVOLUTIONAL){
            forward_convolutional_layer_stripped_mp(l, state, nW, nThreads);
...
void forward_network_stripped(network net, network_state state, int nW)
{
    state.workspace = net.workspace;
    int i;
    for(i = 0; i < net.n; ++i){
        ...
        if(l.type == CONVOLUTIONAL){
            forward_convolutional_layer_stripped(l, state, nW);
        } else if(l.type == DECONVOLUTIONAL){
            forward_deconvolutional_layer(l, state);
        } else if(l.type == ACTIVE){
            forward_activation_layer(l, state);
...
void forward_network_mp(network net, network_state state, int nThreads)
{
    state.workspace = net.workspace;
    int i;
    for(i = 0; i < net.n; ++i){
        ...
        if(l.type == CONVOLUTIONAL){
            forward_convolutional_layer_mp(l, state, nThreads);
        } else if(l.type == DECONVOLUTIONAL){
            forward_deconvolutional_layer(l, state);
        } else if(l.type == ACTIVE){
            forward_activation_layer(l, state);
...
float *network_predict_mp(network net, float *input, int nThreads)
{
#ifdef GPU
    ...
    state.train = 0;
    state.delta = 0;
    forward_network_mp(net, state, nThreads);
    float *out = get_network_output(net);
    return out;
}
...
float *network_predict_stripped_mp(network net, float *input, int nW, int nThreads)
{
#ifdef GPU
    if(gpu_index >= 0)  return network_predict_gpu(net, input);
#endif
    ...
    state.train = 0;
    state.delta = 0;
    forward_network_stripped_mp(net, state, nW, nThreads);
    float *out = get_network_output(net);
    return out;
}

float *network_predict_stripped(network net, float *input, int nW)
{
#ifdef GPU
    if(gpu_index >= 0)  return network_predict_gpu(net, input);
#endif
    ...
    state.train = 0;
    state.delta = 0;
    forward_network_stripped(net, state, nW);
    float *out = get_network_output(net);
    return out;
}
